...
async def _admin_filters(_, client, message: Message) -> bool:
    if message.chat.type != "private":
        ...
        return bool(user_id in await adminlist(client, chat_id)
...
async def staff_rank(flt, client, message: Message) -> bool:
    ...
    if flt.rank == "dev":
        return bool(user_id in client.__bot__.staff.get("dev")
                    or user_id == client.__bot__.staff.get("owner"))
    LOGGER.error("Unknown rank '%s'! Avalaible rank ['owner', 'dev']",
                 flt.rank)
    return False
...
async def check_perm(flt, client, message: Message) -> bool:
    ...
    if message.chat.type == "private":
        return await message.reply_text(await client.__bot__.text(
            chat_id, "error-chat-private"))
    ...
    if flt.can_change_info and not (bot.can_change_info
                                    and user.can_change_info):
        perm = False
    elif flt.can_delete and not (bot.can_delete_messages
                                 and user.can_delete_messages):
        perm = False
    elif flt.can_restrict and not (bot.can_restrict_members and
                                   (user.can_restrict_members
                                    or user in client.__bot__.staff_id)):
        perm = False
    elif flt.can_invite_users and not (bot.can_invite_users
                                       and user.can_invite_users):
        perm = False
    elif flt.can_pin and not (bot.can_pin_messages and user.can_pin_messages):
        perm = False
    elif flt.can_promote and not (bot.can_promote_members and
                                  (user.can_promote_members
                                   or user in client.__bot__.staff_id)):
        perm = False
